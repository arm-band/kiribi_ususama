...
- 2018/8/23 ver.3.7.3-4.1.3
    - class, idを整理
    - スタイルガイドを最新に反映
- 2018/8/21 ver.3.7.2-4.1.3
    - Font Awesomeのバージョンを4.7.0から5.2.0へアップデート
- 2018/8/16 ver.3.7.1-4.1.3
    - ハンバーガーメニューの`data-target`属性の値を書き換え忘れていてトグルが動かなくなっていたバグを解消
- 2018/8/15 ver.3.7.0-4.1.3
    - gulpのバージョンを3.9.1から4.0.0にアップデートしました
- 2018/8/10 ver.3.6.1-4.1.3
    - `index.js`のページ内スクロールの処理の書き方を変更
- 2018/8/8 ver.3.6.0-4.1.3
    - `baseurl`(`base href`)によるパス表記ではなく、相対パスを自動付与する方式にパス指定の方法を変更
        - `news.ejs`や`article.ejs`以外は通常の記述方法で良い
        - 新着記事中の画像やリンクのパスはサイトのルートディレクトリから始まるようにパスを記述すれば良い
- 2018/8/8 ver.3.5.4-4.1.3
    - `\`(バックスラッシュ)を`/`(スラッシュ)に置換する前処理を挟むことで、OSによるパスの指定(`/` or `\`)を削除
- 2018/8/8 ver.3.5.3-4.1.3
    - いくつかの修正
        - `gulpfile.js`の整理
        - Bootstrapではホバー時に色が暗くなるので、追加分のscssでもホバー時に明度が低くなるように調整
- 2018/8/6 ver.3.5.2-4.1.3
    - いくつかの修正
...
- 2018/7/6 ver.3.2.1-4.0.0
    - 記法やファイルを整理、`gulpfile.js`の整理など
    - 新着情報一覧に表示する1ページ当たりの新着情報の件数を3カラムのカードに併せて5から6に変更
- 2018/7/6 ver.3.2.0-4.0.0
